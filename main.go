...
func main() {
	fmt.Println(
		"1. Ввести в одну строку два числа и математический знак между ними.\n" +
			"2. Можно использовать как арабские так и римские числа, оба числа должны быть в одной системе исчисления\n" +
			"3. Калькулятор поддерживает только 4 математических операции: '+', '-', '*', '/'\n" +
			"\n\n" +
			"Введите выражение:")
	...
	if intType == "arab" {
		...
		if err1 != nil {
			fmt.Println("Ошибка при переводе строки в число:\n", err1)
			return
		}
		...
		if err3 != nil {
			fmt.Println("Ошибка при работе калькулятора:\n", err3)
			return
		} else {
...
var errorDict = map[int]string{
	...
	6: "Введите только два числа и один математический оператор",
	7: "В римской системе нет отрицательных чисел",
